...
var getObject = function(resourceType, objectType, objectId, callback) {
    ...
    var options = {
        ...
        auth: common.auth,
        qs:_.extend({}, common.queryParams, {
            Type:objectType,
            Id:objectId,
...
var getPhotos = function(resourceType, photoType, matrixId, callback) {
    getObject(resourceType, photoType, matrixId+":*", function(error, contentType, data) {
        ...
        var multipartBoundary = contentType.match(/boundary=(?:"([^"]+)"|([^;]+))/i);
        multipart.parseMultipart(new Buffer(data), multipartBoundary[1] || multipartBoundary[2], function(error, dataList) {
